...
const dummyComplaints = [
  { 
    ...
    description: 'The bathroom tap has been continuously leaking since yesterday.', 
  ...
  { 
    ...
    description: 'Cannot connect to the boarding Wi-Fi network from my room.', 
    category: 'Utilities', 
    priority: 'High',
    status: 'Resolved',
    date: '2025-05-18',
...
export default function ComplaintsScreen({ navigation, route }) {
  ...
  useEffect(() => {
    if (route.params?.newComplaint) {
      const newCmp = route.params.newComplaint;
      
      if (route.params.isEdit) {
        setComplaints(complaints.map(c => c.id === newCmp.id ? newCmp : c));
      } else {
        setComplaints([newCmp, ...complaints]);
      }
      
      navigation.setParams({ newComplaint: null, isEdit: null });
    }
  }, [route.params?.newComplaint]);

  const handleDelete = (id) => {
    Alert.alert("Delete Complaint", "Are you sure you want to withdraw this complaint?", [
      { text: "Cancel", style: "cancel" },
      { 
        text: "Delete", 
        style: "destructive", 
        onPress: () => setComplaints(complaints.filter(c => c.id !== id))
      }
    ]);
  };
  ...
  const renderComplaintCard = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.tagRow}>
        ...
        <Text style={styles.priorityText}>{getPriorityIcon(item.priority)} {item.priority} Priority</Text>
      </View>
